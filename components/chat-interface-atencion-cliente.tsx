...
import UserList from "./user-list-atencion-cliente"
import ChatWindow from "./chat-window-atencion-cliente"
import NotificationPopup from "./notification-popup-atencion-cliente"
...
export default function ChatInterfaceAtencionCliente() {
  ...
  const isContactoEntrantePage = pathname?.includes("/atencion-cliente/gestion/agenda/contacto-entrante/")
  ...
  const isInSpeechTab = activeTab === "speech" // Si es la pestaña speech

  // Reducir la altura en un 50% (de 200px a 100px)
  const chatButtonPosition = isContactoEntrantePage ? (isInSpeechTab ? "bottom-[100px]" : "bottom-4") : "bottom-4"
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[100] flex flex-col items-end">
      ...
      {showNotification && notificationData && (
        <div className={isSpeechTabActive ? "relative top-[-40px]" : ""}>
          <NotificationPopup
